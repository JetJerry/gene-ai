...
image = (
    ...
    # Build deps
    .pip_install("packaging", "wheel", "setuptools", "numpy")
    ...
    # Evo2
    .pip_install("evo2")
    .pip_install("fastapi", "uvicorn", "requests", "pandas", "numpy", "matplotlib", "seaborn", "scikit-learn", "biopython", "openpyxl")
)

app = modal.App("variant-analysis-evo2", image=image)
...
def analyze_variant(relative_pos_in_window, reference, alternative, window_seq, model):
    ...
    scores = model.score_sequences([window_seq, var_seq])
    ref_score, var_score = scores
...
class Evo2Model:
    ...
    def analyze_single_variant(self, request: VariantRequest):
        ...
        if relative_pos < 0 or relative_pos >= len(window_seq):
            raise ValueError(f"Variant position {variant_position} is outside the fetched window")
...
def main():
    ...
    print(f"Reference: {result['reference']} -> Alternative: {result['alternative']}")
